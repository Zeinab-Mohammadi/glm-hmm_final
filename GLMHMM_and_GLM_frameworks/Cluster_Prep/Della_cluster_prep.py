...
if __name__ == '__main__':
    ...
    #                 cluster_job_arr.append([num_inputs, K, i, j])
        ...
        for prior_sigma in prior_sigmas:
            for transition_alpha in transition_alphas:
                for K in K_vals:
                    for i in range(cross_valid_num_fold):
                        for j in range(model_init_num):
                            cluster_job_arr.append([prior_sigma, transition_alpha, K, num_inputs, i,
                                                    j])
    print('len(cluster_job_arr) = ', len(cluster_job_arr))

    np.savez('../../glm-hmm_package/data/ibl/Della_cluster_data/separate_mouse_data/cluster_job_arr.npz',
             cluster_job_arr)
