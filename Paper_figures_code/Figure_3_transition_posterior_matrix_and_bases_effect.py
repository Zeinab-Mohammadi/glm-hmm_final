...
from analyze_results_utils import makeRaisedCosBasis, get_file_name_for_best_model_fold, permute_transition_matrix, find_corresponding_states_new, calculate_posterior_given_data, data_segmentation_session, mask_for_violations
...
if __name__ == '__main__':
    ...
    plt.subplots_adjust(left=0.3, bottom=0.3, right=0.95, top=0.9)
    ...
    for k in range(K):
        time_trace = np.array(
            (weight_vectors_trans[k][3] * (cosBasis[:, 0])) + (weight_vectors_trans[k][4] * (cosBasis[:, 1])) + (weight_vectors_trans[k][5] * (cosBasis[:, 2])))
