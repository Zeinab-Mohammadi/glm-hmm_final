...
from analyze_results_utils import get_file_name_for_best_model_fold, get_global_weights,  \
    calculate_posterior_given_data, \
    data_segmentation_session, mask_for_violations
from Review_utils import get_file_name_for_best_model_fold_GLM_O, get_marginal_posterior
...
if __name__ == '__main__':
    ...
    figure_dir = '../../glm-hmm_package/results/figures_for_paper/fig_reviews/Rev1_bias_blocks_switch_all_animals_no_GLM-T/'
    ...
    for z, animal in enumerate(mice_names):
        ...
        perm = range(K)
        globe = False
        posterior_probs = get_marginal_posterior(globe, inputs, datas, Params_model, K, perm, alpha_val,
                                             prior_sigma)
        ...
        for t in switch_trials:
            if left_probs[t] == 0.8:  # means we are in left block
                ...
                if a[np.where(a > 0)].shape[0] > 0:  # so it has elements
                    pos_values = np.min(a[np.where(a > 0)]) - 1
        ...
        trial_diff = np.array(trial_diff)[(np.array(trial_diff) < 21) & (np.array(trial_diff) > 0)]  # to avoid random transition
    ...
    median_value = np.median(trial_diff_all)  # Compute from original data
    max_value = np.max(trial_diff_all)  # Compute from original data

    trial_diff_final = trial_diff_all  # Assuming trial_diff_all is a list or numpy array
    ...
    plt.title('Model without GLM-T', fontsize=9)
