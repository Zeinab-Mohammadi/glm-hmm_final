...
from analyze_results_utils import makeRaisedCosBasis, get_file_name_for_best_model_fold, permute_transition_matrix, find_corresponding_states_new
...
if __name__ == '__main__':
    ...
    path_analysis = '../../glm-hmm_package/results/model_global_ibl/' + 'num_regress_obs_' + str(
        num_inputs) + '/'
    figure_dir = '../../glm-hmm_package/results/figures_for_paper/Supp_figures/figure_4/'
    ...
    weight_vectors_trans_K4 = weight_vectors_trans.copy()
    ...
    for k in range(K):
        plt.plot(range(M_trans - 2), weight_vectors_trans[k][3:6], color=cols[k], marker='D', markersize=5, lw=1,
                 alpha=0.9)

    plt.xticks(list(range(0, len(figure_covariates_names_trans[3:6]))), figure_covariates_names_trans[3:6],
    ...
    fig.savefig(figure_dir + 'Trans_basis_K=' + str(K) + '.pdf')

    # ====== bases weights for K=5 =======
    fig = plt.figure(figsize=(3.4, 3))
    plt.subplots_adjust(left=0.3, bottom=0.4, right=0.8, top=0.9)
    K = 5
    cols = colors_func(K)
    ...
    for i in range(K - 1):
        trans_weight_append_zero_standard[i, :] = v1 + trans_weight_append_zero[i, :]  # Here we have vi = v1 + wi
    weight_vectors_trans = trans_weight_append_zero_standard[perm]
    weight_vectors_trans_K5 = weight_vectors_trans.copy()
    ...
    fig = plt.figure(figsize=(3.4, 3))
    plt.subplots_adjust(left=0.25, bottom=0.4, right=0.8, top=0.9)
    for base in range(bias_num):
        print('cosBasis[:, base]=', cosBasis[:, base])
        plt.plot(range(cosBasis.shape[0]), cosBasis[:, base], color='gray', ls="--", lw=1)
    plt.ylabel("Bases traces", fontsize=10)
    plt.xlabel("Trial #", fontsize=10)
    plt.gca().spines['right'].set_visible(False)
    plt.gca().spines['top'].set_visible(False)
    fig.savefig(figure_dir + 'cosBasis' + '.pdf')
