...
from io_utils import get_mouse_info_all, cross_validation_vector, ewma_time_series, model_data_glmhmm, get_mouse_info, mice_names_info, load_Pt, load_reward_data, calculate_correct_ans
...
if __name__ == '__main__':
    ...
    dir_for_save = '../../glm-hmm_package/results/figures_for_paper/Supp_figures/figure_5/with GLM-T/sim_data/'
    figure_dir = '../../glm-hmm_package/results/figures_for_paper/Supp_figures/figure_5/with GLM-T'
    ...
    for j in range(num_simulations):
        ...
        for z, animal in enumerate(mice_names):
            ...
            for i, input in enumerate(inputs):  # for each session
                ...
                # updating input_trans: [pc, stim_side, pre_reward, basis1:3]
                tau = 4
                past_reward_value = (2*bernoulli.rvs(0.5, size=1)) - 1
                input_trans[0, 0] = ewma_time_series(pc, tau)[0]
                input_trans[0, 1] = ewma_time_series(stim_side, tau)[0]
                input_trans[0, 2] = ewma_time_series(past_reward_value, tau)[0]
                ...
                for t in range(0, T):
                    ...
                    data[t] = this_hmm.observations.sample_x(latent_z[t], xhist=None, observation_input=np.expand_dims(input[t], axis=0), tag=None)
                    ...
                    if t < T-1:
                        ...
                        # for transition
                        pc_session.append(pc)
                        stim_side_session.append(stim_side)
                        pre_reward_session.append(rewarded)
                    trial_num += 1
                # for transition inputs
                input_trans[1:T, 0] = ewma_time_series(pc_session, tau)[0]  # pc
                input_trans[1:T, 1] = ewma_time_series(stim_side_session, tau)[0]  # stim_side
                input_trans[1:T, 2] = ewma_time_series(pre_reward_session, tau)[0]  # prev_reward
            ...
            np.savez(animal_dir_for_save + '/simulation_' + str("%03d" % j) + '.npz', obs_mat, trans_mat,
