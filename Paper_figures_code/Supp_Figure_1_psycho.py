...
from io_utils import calculate_correct_ans, get_mouse_info, cross_validation_vector, model_data_glmhmm, get_mouse_info_all, colors_func, find_change_points, get_mouse_info_unorm, get_prob_right, get_prob_right_trans, load_reward_data
from analyze_results_utils import get_file_name_for_best_model_fold, find_corresponding_states_new, calculate_posterior_given_data, data_segmentation_session, mask_for_violations, calculate_posterior_given_data_post
...
if __name__ == '__main__':
    ...
    globe = True
    posterior_probs, Ps_all = calculate_posterior_given_data(globe, inputs, inputs_trans, datas, Params_model, K, alpha_val,
                                                     prior_sigma, range(K))
    ...
    # --- collect per-state psychometric curves for the source-data sheet ---
    state_labels = ["Engaged-L", "Engaged-R", "Biased-L", "Biased-R", "Past-choice"]
    curve_dict = {lab: None for lab in state_labels}
    ...
    for k in range(K):
        ...
        curve_dict[state_labels[k]] = np.asarray(prob_right_max, dtype=float)
        ...
        if k == 3:
            ...
            plt.gca().tick_params(axis='x')
            plt.gca().tick_params(axis='y')
            plt.text(-1, .65, "Biased-R", fontsize=9, color=cols[k])
        if k == 4:
            plt.subplot(1, 3, 2)
            plt.xticks([min(stim_vals), min(stim_vals) / 2, 0, max(stim_vals) / 2, max(stim_vals)],
                       labels=['', '', '', '', ''], fontsize=8)
            plt.yticks([0, 0.25, 0.5, 0.75, 1], ['', '', '', '', ''], fontsize=8)
            plt.gca().tick_params(axis='x')
            plt.gca().tick_params(axis='y')
            plt.text(-2.1, .9, "Past-choice", fontsize=9, color=cols[k])
    ...
    # sns.lineplot(data=obs_mat_df, x="signed_contrast", y="choice",
    #              err_style="bars", err_kws={"linewidth": 0.75}, mew=0, color=cols[2], errorbar=('ci', 68),
    #              label="Without GLM-T", alpha=1, lw=0.75)
